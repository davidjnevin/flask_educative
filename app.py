...
from forms import EditPetForm, LoginForm, SignUpForm
...
with app.app_context():
    ...
    mrfurrkins = Pet(name="Mr. Furrkins", age="5 years", bio="More than likely, probably napping.")
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.rollback()
            return render_template(
                "signup.html", form=form, message="This Email already exists in the system! Please Log in instead."
            )
        finally:
            db.session.close()
